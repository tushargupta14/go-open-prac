...
func valueIsValidString(values []string, s string) bool {
	...
	copy(v, values)
	sort.Strings(v)
	return sort.SearchStrings(v, s) < len(v)
}
...
func valueIsValidInt(values []int, s int) bool {
	...
	copy(v, values)
	sort.Ints(v)
	return sort.SearchInts(v, s) < len(v)
}
...
func TestDeaf(t *testing.T) {
	...
	pxa[0].Start(1, "xxx")
	waitn(t, pxa, 1, npaxos-1, []string{"xxx"})
	time.Sleep(1 * time.Second)
	if ndecided(t, pxa, 1, []string{"xxx"}) != npaxos-1 {
		t.Fatalf("a deaf peer heard about a decision")
	}

	pxa[npaxos-1].Start(1, "yyy")
	waitn(t, pxa, 1, npaxos, []string{"yyy"})
